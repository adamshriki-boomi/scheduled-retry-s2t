...
export function BaseLimitationContent({
  toggleModal,
  openHelpModal,
    ...
    <ModalContent maxW="lg">
      <ModalHeader py={2}>
        <HStack justify="space-between">
          <Text textStyle="M6">
            Thanks for subscribing to the Boomi base plan
          </Text>
          <CloseIconButton
            onClick={toggleModal}
            aria-label="close"
      ...
      <ModalBody>
        <Text>
          Our Base Plan offers unlimited Data Sources, Destinations,
          ...
          <OrderedList pt={3}>
            <ListItem>
              <Flex flexDir="column" gap={2}>
                <Box>
                  Stay on the Base Plan, select the Environment you would like
                  to work in, and{' '}
                  <RiveryButton
                    label="deploy"
                    variant="link"
            ...
            <ListItem mt={6}>
              <Box>
                Upgrade to the Boomi Data Integration{' '}
                <strong>Professional Plan</strong> and benefit from access to{' '}
                <RenderGuard condition={Boolean(tokens)}>
...
function Alert({ text }) {
  const { basePlanLimitations } = useCore();
  return (
    <RiveryAlert
      ...
      description={
        <Box>
          ...
          <DateDisplay value={basePlanLimitations.expDate} display="inline" /> ,{' '}
          {text}
        </Box>
      }
    />
