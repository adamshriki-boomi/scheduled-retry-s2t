...
import { DistributionMethodTypes, TargetTypesV1 } from 'api/types';
...
export function AllColumns({ isDisabled }) {
  ...
  const columnsWithApi = baseColumns.map(column => ({
    ...
    getProps: {
      ...api,
      isDisabled,
      targetType: TargetTypesV1.AZURE_SQL_DWH,
      distValues: {
        dist_key: DistributionMethodTypes.HASH,
      },
    },
    ...
    >
      ...
      {mappingDraft ? (
        <ColumnMappingEditor
          ...
          onChange={column => {
            closeEditor();
            api.updateColumn(column.name, {
              ...column,
              //temp fix waiting for proper BE fix
              length: column.type === 'VARCHAR' ? column.length : null,
            });
          }}
